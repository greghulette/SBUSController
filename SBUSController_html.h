...
class SerialTransport extends Transport {
  ...
  // Tear it down and report honestly.  No auto-reopen: after a suspend the
  // SerialPort handle is usually stale and needs a fresh pick, which Web Serial
  // only allows from a user gesture — so the user re-picks USB Serial.
  async _linkLost() {
    if (!this.connected) return;
    console.warn('[transport] USB Serial link lost (no data for ' + LINK_TIMEOUT_MS + ' ms)');
    const wasActive = (transport === this);
    try { await this.close(); } catch (_) {}
    if (wasActive) this.onstate(false);
  }
  ...
  probe() {
    ...
    this.send({ t: 'ping' });
    setTimeout(() => {
      if (transport === this && Date.now() - this._lastRxMs > LINK_PROBE_MS) this._linkLost();
    }, LINK_PROBE_MS);
  }
...
function handleBootLog(msg) {
  ...
  sum.innerHTML =
    ...
    `<span style="color:var(--muted);"> (code ${msg.rst}, RTC ${msg.rtc0})</span></div>` +
  ...
  (msg.log || []).forEach((b, i) => {
    ...
    tr.innerHTML =
      ...
      `<td style="padding:4px 8px;color:var(--muted);">${b.rtc0}</td>` +
