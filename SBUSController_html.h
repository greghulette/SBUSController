...
<!-- Inline SVG favicon — a transmitter with two gimbal sticks + antennas, in
     the app's blue palette.  Data-URI so no separate file is served (the
     firmware ships this HTML as one flash blob) and it works on GitHub Pages
     too.  '#' in hex colours is %23-encoded as data URIs require. -->
<link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'%3E%3Crect x='2' y='8' width='28' height='19' rx='5' fill='%231f3870' stroke='%234fc3f7' stroke-width='1.5'/%3E%3Cline x1='8' y1='8' x2='6' y2='2.5' stroke='%234fc3f7' stroke-width='1.6' stroke-linecap='round'/%3E%3Cline x1='24' y1='8' x2='26' y2='2.5' stroke='%234fc3f7' stroke-width='1.6' stroke-linecap='round'/%3E%3Ccircle cx='10' cy='17.5' r='4.2' fill='%230b1020' stroke='%237acdf6' stroke-width='1.2'/%3E%3Ccircle cx='10' cy='17.5' r='1.7' fill='%234fc3f7'/%3E%3Ccircle cx='22' cy='17.5' r='4.2' fill='%230b1020' stroke='%237acdf6' stroke-width='1.2'/%3E%3Ccircle cx='22' cy='17.5' r='1.7' fill='%234fc3f7'/%3E%3C/svg%3E">
<style>
  :root {
    --bg:       #0d0f14;
    --panel:    #161920;
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  /* Link dropped but we're actively retrying — amber, and pulsing so a glance
     at the header tells you the page is stale without having to read it. */
  #connBadge.reconnecting{border-color:var(--yellow);color:var(--yellow);animation:badgePulse 1.4s ease-in-out infinite;}
...
<!-- ── Diagnostics ──────────────────────────────────────────────────────────────
     Reset history read out of the board's RTC RAM.  Deliberately NOT inside
     fwWrap: this has to work on the ESP-served page too, and it's most needed
     over USB Serial — where the UI owns the port and a serial terminal can't
     be attached to watch the boot banner. -->
<div class="settings-wrap">
  <details id="diagDetails" ontoggle="if(this.open) requestBootLog()">
    <summary>Diagnostics</summary>
    <div class="settings-body">
      <div style="font-size:.74rem;color:var(--muted);margin-bottom:10px;line-height:1.5;">
        Why the board last restarted, kept in RTC RAM so it survives every reset except
        losing power. <b style="color:var(--text)">Ran for</b> is how long the previous
        boot lasted before it died — a consistent figure points at something in the
        firmware, a random one usually means power.
      </div>
      <div id="diag-summary" style="font-size:.72rem;font-family:monospace;background:var(--bg);border:1px solid var(--border);border-radius:7px;padding:8px 12px;margin-bottom:10px;line-height:1.7;">
        <span style="color:var(--muted);">Connect to read boot history.</span>
      </div>
      <div id="diag-log-wrap" style="display:none;overflow-x:auto;">
        <table id="diag-log" style="width:100%;border-collapse:collapse;font-size:.7rem;font-family:monospace;">
          <thead><tr>
            <th style="text-align:left;padding:4px 8px;color:var(--muted);font-weight:600;">#</th>
            <th style="text-align:left;padding:4px 8px;color:var(--muted);font-weight:600;">Reset reason</th>
            <th style="text-align:left;padding:4px 8px;color:var(--muted);font-weight:600;">RTC</th>
            <th style="text-align:left;padding:4px 8px;color:var(--muted);font-weight:600;">Ran for</th>
          </tr></thead>
          <tbody></tbody>
        </table>
      </div>
      <div style="margin-top:10px;">
        <button class="wifi-apply" onclick="requestBootLog()">&#8635; Refresh</button>
      </div>
    </div>
  </details>
</div>
...
<!-- ─── Support Footer (NaviCore / WCB style) ───────────────────────────────
     Images live in docs/images/ and only exist on the GitHub Pages / local
     copy; the ESP-served WiFi page can't serve them, so each <img> has an
     onerror that hides it — the text + link buttons still render fine there. -->
<div style="max-width:960px;width:100%;margin:48px auto 0;border-top:1px solid var(--border);padding:32px 12px 28px;display:flex;align-items:center;justify-content:space-between;gap:32px;flex-wrap:wrap">
  <div style="display:flex;align-items:center;gap:18px">
    <a href="https://beacons.ai/greghulette" target="_blank" rel="noopener" title="Scan for all contact links">
      <img src="images/qr-code.png" alt="Contact QR code" onerror="this.style.display='none'"
           style="width:72px;height:72px;border-radius:5px;display:block;opacity:.85;transition:opacity .15s"
  ...
  <div style="display:flex;flex-direction:column;align-items:center;gap:8px;text-align:center">
    <img src="images/r2logo.png" alt="R2 logo" onerror="this.style.display='none'"
         style="width:64px;height:64px;object-fit:contain;opacity:.9">
    <div style="font-size:11px;color:var(--muted);font-family:monospace;letter-spacing:.3px;line-height:1.7">
      SBUS Controller Config Tool<br>
      UI Version <span id="footer-ui-version">—</span>
    </div>
  </div>
  <div style="display:flex;align-items:center;gap:10px;flex-shrink:0;flex-wrap:wrap">
    <a href="https://github.com/greghulette/SBUSController" target="_blank" rel="noopener"
       style="display:inline-flex;align-items:center;gap:7px;padding:7px 14px;background:var(--panel);
              border:1px solid var(--border);border-radius:6px;text-decoration:none;font-size:12px;
              font-weight:600;color:var(--text);letter-spacing:.3px;transition:border-color .15s,color .15s"
    ...
    <a href="https://discord.gg/AdJPxc8QvZ" target="_blank" rel="noopener"
       style="display:inline-flex;align-items:center;gap:8px;padding:7px 14px;background:#5865F2;
              border-radius:6px;text-decoration:none;font-size:12px;font-weight:600;color:#fff;
...
// ── Link liveness ───────────────────────────────────────────────────────────
// Neither transport reports a dropped link reliably on its own.  A WebSocket
// only fires onclose when the TCP connection is torn down cleanly; suspend the
// laptop, drop off the AP, or pull the board's power and the socket is left
// HALF-OPEN — readyState stays OPEN, no event fires, and the UI happily shows
// CONNECTED against a board that isn't there.  (The browser's own TCP timeout
// can take many minutes to notice.)  Web Serial has the same hole: a port
// handle can survive a suspend and simply never deliver another byte.
//
// So we don't trust the socket — we drive our own heartbeat.  Every transport
// PINGs on a fixed cadence and stamps _lastRxMs on ANY inbound traffic; if
// nothing at all comes back within LINK_TIMEOUT_MS the link is declared dead.
// The firmware answers {t:"ping"} with a PONG on both transports, so an idle
// but healthy link still produces traffic and never trips the watchdog.
const LINK_PING_MS    = 3000;   // heartbeat cadence
const LINK_TIMEOUT_MS = 9000;   // three missed heartbeats ⇒ link is dead
const LINK_PROBE_MS   = 2500;   // shorter deadline for an on-wake probe
...
class Transport {
  constructor() {
    ...
    this.onstate   = (_connected, _detail) => {}; // connection state change
  }
  open()           { /* override */ }
  close()          { /* override */ }
  // Returns TRUE only if the message actually went out.  Callers that change
  // saved state (wifiApply, saveConfig) must check it — a send that silently
  // evaporates while the link is down looks exactly like a save that didn't
  // stick, which is far worse than an error message.
  send(_obj)       { return false; }
  isConnected()    { return false; }
  probe()          { /* override — force an immediate liveness check */ }
}
...
class WsTransport extends Transport {
  constructor() {
    ...
    this._hbTimer  = null;   // heartbeat + watchdog interval
    this._lastRxMs = 0;      // stamped by every inbound frame (PONG or broadcast)
  }
  open() {
  ...
  // Guarded so a close event and a watchdog trip can't stack two timers.
  _scheduleReconnect() {
    if (this._reconnectTimer) return;
    this._reconnectTimer = setTimeout(() => {
      this._reconnectTimer = null;
      if (transport === this) this.open();
    }, 3000);
  }
  ...
  _startHeartbeat() {
    ...
    this._hbTimer = setInterval(() => {
      ...
      if (Date.now() - this._lastRxMs > LINK_TIMEOUT_MS) { this._linkLost(); return; }
      this.send({ t: 'ping' });
    }, LINK_PING_MS);
  }
  _stopHeartbeat() { if (this._hbTimer) { clearInterval(this._hbTimer); this._hbTimer = null; } }

  // Drop a half-open socket and start over.  close() on a half-open socket may
  // never fire onclose, so we detach the handlers and drive the reconnect
  // ourselves rather than waiting on an event that isn't coming.
  _linkLost() {
    this._stopHeartbeat();
    if (this.ws) {
      this.ws.onopen = this.ws.onclose = this.ws.onerror = this.ws.onmessage = null;
    ...
    console.warn('[transport] WiFi link lost (no traffic for ' + LINK_TIMEOUT_MS + ' ms)');
    this.onstate(false, 'reconnecting');
    this._scheduleReconnect();
  }

  // Called when the page wakes up (tab re-shown, machine resumed from sleep).
  // A socket that slept through a suspend is almost always dead, so don't make
  // the user stare at a stale CONNECTED for a full timeout — ping now and hold
  // it to the shorter probe deadline.
  probe() {
    if (this.ws && this.ws.readyState === WebSocket.CONNECTING) return;   // already trying
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      // Nothing open: pull the retry forward instead of waiting out its timer.
      if (this._reconnectTimer) { clearTimeout(this._reconnectTimer); this._reconnectTimer = null; }
      if (transport === this) this.open();
      return;
    }
    this.send({ t: 'ping' });
    setTimeout(() => {
      if (transport === this && this.ws && Date.now() - this._lastRxMs > LINK_PROBE_MS) this._linkLost();
    }, LINK_PROBE_MS);
  }
}
...
class SerialTransport extends Transport {
  constructor() {
    ...
    this._closing  = false;
    this._lastRxMs = 0;            // stamped by every inbound byte — see LINK_TIMEOUT_MS
    this._stalled  = false;        // open but silent; recovers on its own, never closes
  }
  ...
  async open(existingPort = null) {
    ...
    try {
      ...
      // firmware echoes broadcastJson() to Serial.  The same beat doubles as
      // the liveness watchdog: an unplugged cable usually ends the read loop,
      // but a suspended machine or a wedged board can leave the port open and
      // silent — the PONG that stops arriving is what catches that.
      this.send({ t: 'ping' });
      // Keep pinging even while stalled — the ping is also how we notice the
      // port coming back, and it costs nothing on a port that's already open.
      this.pingTimer = setInterval(() => {
        if (Date.now() - this._lastRxMs > LINK_TIMEOUT_MS) this._markStalled();
        this.send({ t: 'ping' });
      }, LINK_PING_MS);
  ...
  async _readLoop() {
    ...
    try {
      while (true) {
        ...
        if (done) break;
        this._lastRxMs = Date.now();   // any byte proves the port is still live
        // Bytes resumed after a silent spell — recover without touching the
        // port (see _markStalled on why closing it is never the answer).
        if (this._stalled) {
          this._stalled = false;
          console.log('[transport] USB Serial recovered');
          if (transport === this) this.onstate(true);
        }
        buf += value;
        let idx;
  ...
  send(obj) {
    if (!this.writer || !this.connected) return false;
  ...
  // Port is open but has gone silent (suspend, wedged board, half-dead CDC).
  // Report it — but do NOT close the port.
  //
  // Closing is not a neutral act on this hardware.  On the S3's native USB CDC,
  // releasing the port toggles DTR/RTS, and the USB-Serial/JTAG peripheral
  // treats that as a reset request: it REBOOTS THE BOARD (shows up as reset
  // reason ESP_RST_USB / RTC code 21 USB_UART_CHIP_RESET).  An earlier version
  // of this watchdog closed on suspicion, which meant a single false positive
  // didn't just mis-report a healthy link — it reset a running board, and the
  // reset then made the dead link real.  Never close on suspicion.
  //
  // Nothing is lost by staying open.  A genuinely dead port ends the read loop
  // with EOF, and _readLoop's finally reports that already.  This path only
  // covers "open but silent", and leaving the port up lets it self-heal: the
  // read loop is still running, so if bytes resume we go straight back to
  // connected without the user touching anything.
  _markStalled() {
    if (this._stalled || !this.connected) return;
    this._stalled = true;
    console.warn('[transport] USB Serial silent for ' + LINK_TIMEOUT_MS +
                 ' ms — port left OPEN (closing it would reset the board)');
    if (transport === this) this.onstate(false, 'reconnecting');
  }

  // Force an immediate liveness check on wake — same idea as WsTransport.probe,
  // except this one can only ever downgrade the badge, never touch the port.
  probe() {
    if (!this.connected) return;
    this.send({ t: 'ping' });
    setTimeout(() => {
      if (transport === this && Date.now() - this._lastRxMs > LINK_PROBE_MS) this._markStalled();
    }, LINK_PROBE_MS);
  }
...
function handleInboundMessage(msg) {
  ...
  else if (msg.e === 'bootlog')        handleBootLog(msg);
  else if (msg.t === 'pong')           handlePong(msg);
}

// ── Diagnostics: boot history ───────────────────────────────────────────────
// The board keeps its reset history in RTC RAM (see buildBootLogJson in the
// firmware) precisely because the serial banner is unreadable while this page
// owns the USB port.  Pulled on connect so an abnormal reset is flagged in the
// header even with the panel closed.
function requestBootLog() { send({ t: 'bootlog' }); }

const _fmtRan = (ms) => !ms ? '—'
  : ms < 60000 ? (ms / 1000).toFixed(1) + ' s'
  : Math.floor(ms / 60000) + 'm ' + Math.round((ms % 60000) / 1000) + 's';

// Serial-link forensics row. Only meaningful on the USB transport, and only
// worth showing once there's history to read.
//
// The board can't reboot its way out of a dead USB link — it stays up, which is
// exactly why the reset log alone can't explain a dropped connection. The
// longest inbound-silence gap can: the UI pings every LINK_PING_MS, so on a
// healthy link the gap sits just above that. A gap that matches an outage you
// actually saw means inbound died too (host-side USB stall, nothing the
// firmware can do). A gap that stayed small through the outage means the board
// kept hearing us and only its replies were lost.
function _diagSerialLine(msg) {
  if (msg.sgap == null || !(transport instanceof SerialTransport)) return '';
  const gap  = msg.sgap;
  const bad  = gap > LINK_TIMEOUT_MS;
  const verdict = !bad
    ? 'inbound never stalled'
    : 'inbound stalled too &mdash; host-side USB stall';
  return `<div style="color:var(--muted);">Serial link &nbsp;longest inbound gap ` +
         `<b style="color:${bad ? 'var(--yellow)' : 'var(--green)'};">${_fmtRan(gap)}</b>` +
         ` <span style="color:var(--muted);">(${verdict})</span>` +
         ` &nbsp;·&nbsp; pings answered <b style="color:var(--text);">${msg.spng}</b></div>`;
}

function handleBootLog(msg) {
  const sum = document.getElementById('diag-summary');
  const wrap = document.getElementById('diag-log-wrap');
  const tb   = document.querySelector('#diag-log tbody');
  if (!sum || !tb) return;

  // reason 1 = ESP_RST_POWERON. Anything else means the board restarted on its
  // own, which is the case worth shouting about.
  const abnormal = msg.rst !== 1;
  const colour   = abnormal ? 'var(--yellow)' : 'var(--green)';
  sum.innerHTML =
    `<div>Last reset &nbsp;<b style="color:${colour};">${msg.rstn || '?'}</b>` +
    `<span style="color:var(--muted);"> (code ${msg.rst}, RTC ${msg.rtc0} ${msg.rtcn || ''})</span></div>` +
    `<div style="color:var(--muted);">Boots since power applied <b style="color:var(--text);">${msg.n}</b>` +
    ` &nbsp;·&nbsp; previous boot ran <b style="color:var(--text);">${_fmtRan(msg.prev)}</b>` +
    ` &nbsp;·&nbsp; up now <b style="color:var(--text);">${_fmtRan(msg.up)}</b></div>` +
    _diagSerialLine(msg);
  ...
  (msg.log || []).forEach((b, i) => {
    ...
    tr.innerHTML =
      `<td style="padding:4px 8px;color:var(--muted);">${i === 0 ? 'now' : '-' + i}</td>` +
      `<td style="padding:4px 8px;color:${b.r === 1 ? 'var(--text)' : 'var(--yellow)'};">${b.rn}</td>` +
      `<td style="padding:4px 8px;color:var(--muted);">${b.rtc0} ${b.rtcn || ''}</td>` +
      `<td style="padding:4px 8px;">${_fmtRan(b.ran)}</td>`;
    tb.appendChild(tr);
  });
  wrap.style.display = (msg.log && msg.log.length) ? 'block' : 'none';

  // Nudge the user toward the panel when the board restarted by itself, and
  // count repeats — an intermittent fault is easy to miss one event at a time.
  if (abnormal) {
    _diagResetCount++;
    const d = document.getElementById('diagDetails');
    const s = d && d.querySelector('summary');
    if (s) {
      s.innerHTML = `Diagnostics <span style="color:var(--yellow);font-weight:600;">` +
                    `⚠ ${msg.rstn}${_diagResetCount > 1 ? ` (×${_diagResetCount} this session)` : ''}</span>`;
    }
  }
}
let _diagResetCount = 0;   // abnormal resets seen since the page was loaded
...
function handlePong(msg) {
  ...
  // PONGs now arrive every LINK_PING_MS on both transports (they're the
  // heartbeat), so only touch the DOM and the log when something actually
  // changed — otherwise this repaints the version chip 20 times a minute.
  if (msg.fwver != null && msg.fwver !== cfg.fwver) {
    cfg.fwver = msg.fwver;
    updateFwVersion();
...
// that changes the page) replaces the __UI_VERSION__ token below with the repo's
// <date>-<shortsha> when it generates docs/index.html. On the ESP-served WiFi
// page the token is never replaced, so we fall back to "dev". Compare this
// (latest build) against the header's FW version (what's on the board) to see
// whether the board needs a flash.
const UI_VERSION = '__UI_VERSION__';
function initFooterVersion() {
  const el = document.getElementById('footer-ui-version');
  if (el) el.textContent = UI_VERSION.startsWith('__') ? 'dev' : UI_VERSION;
}
...
// Single onstate sink for every transport.  Paints the badge, and on each
// (re)connect pulls the board's boot history — that's what turns an
// intermittent self-reset into something you can actually see: the board
// restarts, the transport reconnects, and the new reset reason lands in the
// Diagnostics panel with the session counter bumped.
function onTransportState(on, detail) {
  setBadge(on, detail);
  if (on) requestBootLog();
}
...
function connect() {
  ...
  transport.onstate   = onTransportState;
  _updateTransportBtn();
...
function send(obj) { return transport ? !!transport.send(obj) : false; }

// ── Wake-up probes ──────────────────────────────────────────────────────────
// The heartbeat watchdog catches a dead link on its own, but it can't run while
// the machine is asleep — so after a hibernate you'd sit on a stale CONNECTED
// until the timers restart and the first beat times out.  These events fire the
// moment the page comes back, so we probe immediately instead:
//   visibilitychange — tab refocused, and the reliable "resumed from sleep" tell
//   pageshow         — restored from the browser's back/forward cache
//   online/offline   — the OS gained or lost a network
// Never during a flash: esptool owns the port then, and transport is torn down.
function _probeLink() {
  if (_fwFlashing) return;
  if (transport && transport.probe) transport.probe();
}
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible') _probeLink();
});
window.addEventListener('pageshow', (e) => { if (e.persisted) _probeLink(); });
window.addEventListener('online',  _probeLink);
window.addEventListener('offline', () => {
  // No network at all — say so now rather than waiting for the watchdog.
  if (!_fwFlashing && transport instanceof WsTransport) setBadge(false, 'reconnecting');
});
...
// detail (optional) distinguishes "link dropped, retrying in the background"
// from "not connected at all" — the transports pass 'reconnecting' for the
// former so the badge can show an amber, pulsing state instead of a flat red.
function setBadge(on, detail) {
  const b = document.getElementById('connBadge');
  const retrying = !on && detail === 'reconnecting';
  b.textContent = on ? 'CONNECTED' : (retrying ? 'RECONNECTING…' : 'DISCONNECTED');
  b.className   = on ? 'connected' : (retrying ? 'reconnecting' : '');
...
function applyCfg(msg) {
  ...
  // WiFi
  // Don't overwrite the list the user is part-way through editing — a cfg
  // broadcast can land at any time (another client saving, a reconnect, the
  // getcfg after Save Config) and would otherwise erase typed credentials.
  if (Array.isArray(msg.wifiNets) && !_wifiDirty) cfg.wifiNets = msg.wifiNets;
...
// Set as soon as the user touches an SSID/password field, cleared once the
// list has been sent to the board.  While it's set, an inbound cfg broadcast
// must NOT rebuild these rows: the typed text lives in the DOM until Save, and
// a re-render would silently wipe credentials mid-entry — which reads exactly
// like "the page won't let me add a network".
let _wifiDirty = false;

// force=true rebuilds the rows even when dirty.  Only the row-editing buttons
// pass it, and they call wifiCollect(true) first so the rebuild is seeded from
// what's actually on screen rather than from stale model state.
function renderWifiPanel(force) {
  // Status bar
  ...
  // Network rows — never clobber in-progress edits (see _wifiDirty).
  if (_wifiDirty && !force) return;
  ...
  nets.forEach((net, i) => {
    ...
    // Re-read the on-screen values first, so reordering or deleting one row
    // doesn't discard text typed into the others.
    up.onclick  = () => { wifiCollect(true); const n=cfg.wifiNets;
                          if(i>0){const t=n[i];n[i]=n[i-1];n[i-1]=t;_wifiDirty=true;renderWifiPanel(true);} };
    dn.onclick  = () => { wifiCollect(true); const n=cfg.wifiNets;
                          if(i<n.length-1){const t=n[i];n[i]=n[i+1];n[i+1]=t;_wifiDirty=true;renderWifiPanel(true);} };
    del.onclick = () => { wifiCollect(true); cfg.wifiNets.splice(i,1);
                          _wifiDirty=true; renderWifiPanel(true); };
...
function wifiAddRow() {
  wifiCollect(true);                       // keep what's already typed on screen
  // Cap matches MAX_WIFI_NETS in firmware (8).  Firmware would silently drop
  // anything beyond, so we stop the user here — and say so, rather than having
  // the button do nothing.
  if (cfg.wifiNets.length >= 8) {
    const msg = document.getElementById('wifiMsg');
    msg.style.display = 'inline';
    msg.style.color = 'var(--yellow)';
    msg.textContent = 'Maximum of 8 networks — delete one first.';
    return;
  }
  cfg.wifiNets.push({s:'', p:''});
  _wifiDirty = true;
  renderWifiPanel(true);
}

// Read current row values back into cfg.wifiNets.
// keepEmpty=true retains rows that have no SSID yet — needed for UI-side edits
// (add / delete / reorder) so a half-filled row isn't dropped out from under
// the user.  The send path omits it, because the firmware ignores a blank SSID.
function wifiCollect(keepEmpty) {
...
function wifiApply() {
  ...
  // A send that can't go out used to fail silently, leaving "Connecting…" on
  // screen and the network unsaved — indistinguishable from a save that didn't
  // stick. Report it instead, and keep the rows dirty so nothing is lost.
  if (!send({t:'wificfg', pref: cfg.wifiPref, nets: cfg.wifiNets})) {
    msg.style.color = 'var(--red)';
    msg.textContent = 'Not connected — nothing was sent. Reconnect, then Save again.';
    return;
  }
  _wifiDirty = false;                      // the board owns the list now
  msg.style.color = '';
...
function exportConfig() {
  ...
  // Build the export from the in-memory cfg so it works over BOTH transports.
  // The old fetch('/cfg') hit the ESP's HTTP server, which only exists when the
  // board itself serves the page over WiFi — over USB Serial (page loaded from
  // GitHub Pages / file://) there's no server, so the fetch failed with
  // "No config".  cfg is kept current by applyCfg() on either transport.
  if (!cfg || !Array.isArray(cfg.sw) || !cfg.sw.length) {
    el.style.color = 'var(--red)';
    el.textContent = 'Export failed: no config loaded yet — wait for CONNECTED.';
    return;
  }
  // Mirror the firmware's saved-file shape (the same fields importConfig reads
  // back), dropping runtime-only fields (sw.pos / sl.pct / tr.cur) that aren't
  // part of a saved config.
  const out = {
    ver:    cfg.ver || 3,
    rx: cfg.rx, ry: cfg.ry, ly: cfg.ly, lx: cfg.lx,
    sbus24: cfg.sbus24,
    sw:  cfg.sw.map(s => ({l:s.l, c:s.c, t:s.t, d:s.d, v:s.v})),
    sl:  (cfg.sl  || []).map(s => ({l:s.l, c:s.c})),
    tr:  (cfg.tr  || []).map(t => ({l:t.l, c:t.c, s:t.s, m:t.m, vL:t.vL, vR:t.vR})),
    btn: (cfg.btn || []).map(b => ({l:b.l, c:b.c, v:b.v})),
    lua: (cfg.lua || []).map(b => ({l:b.l, c:b.c, v:b.v, k:b.k})),
    aMin: cfg.aMin, aMax: cfg.aMax, aRev: cfg.aRev,
    wifiPref: cfg.wifiPref,
    wifiNets: (cfg.wifiNets || []).map(n => ({s:n.s, p:n.p})),
    pwm: (cfg.pwm || []).map(p => ({c:p.c})),
    pwmExt: cfg.pwmExt
  };
  try {
...
function importConfig(input) {
  ...
  reader.onload = e => {
    try {
      ...
      // An import deliberately replaces the whole list, so drop the edit guard
      // and let the refresh below repaint the rows from what was imported.
      _wifiDirty = false;
...
function saveConfig() {
  ...
  if (!send(payload)) {
    alert('Not connected to the board — nothing was saved.\n\n' +
          'Reconnect (the button in the header), then Save Config again.');
    return;
  }
...
initFooterVersion(); // show the UI build version in the footer
connect();
</script>
</body>
</html>)rawhtml";
